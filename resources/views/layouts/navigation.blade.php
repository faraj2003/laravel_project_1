<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    <x-nav-link :href="$dashboardRoute" :active="request()->routeIs($isAdmin ? 'admin.dashboard' : 'dashboard')">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ $isAdmin ? route('admin.logout') : route('logout') }}">
                            ...
                            <x-dropdown-link :href="$isAdmin ? route('admin.logout') : route('logout')"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="$dashboardRoute" :active="request()->routeIs($isAdmin ? 'admin.dashboard' : 'dashboard')">
                {{ __('Dashboard') }}
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ $isAdmin ? route('admin.logout') : route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="$isAdmin ? route('admin.logout') : route('logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
